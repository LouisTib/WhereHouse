document.addEventListener("DOMContentLoaded", function () {
    ...
    // ============ MOBILE STATE VARIABLES (Simplified for Tap-to-Move) ============
    // Only used to track the currently selected section for movement
    ...
    function generateGrid() {
        ...
        for (let y = 0; y < 20; y++) {
            for (let x = 0; x < 30; x++) {
                ...
                // Handle click for new section placement (Desktop & Mobile)
                ...
                // Handle Mobile Tap-to-Move Logic
                cell.addEventListener("touchstart", (e) =>
                    handleCellTouch(e, x, y)
                );
    ...
    function renderSections() {
        document.querySelectorAll(".grid-cell").forEach((cell) => {
            ...
            cell.classList.remove("mobile-selected"); // Ensure state is cleaned
        });
        ...
        sections.forEach((section) => {
            if (section.grid_x !== null && section.grid_y !== null) {
                ...
                if (cell) {
                    ...
                    // Reapply visual state if section is currently selected for mobile move
    ...
    function handleDragStart(e, section) {
        // Desktop drag bypasses mobile selection logic
        draggedSection = section;
        e.target.classList.add("dragging");
        // Ensure mobile state is cleared if desktop drag starts
    ...
    // ============ TAP-TO-MOVE HANDLER (Mobile Only) ============

    function handleCellTouch(e, x, y) {
        // Prevent default browser behavior (like scrolling) for the duration of the touch
        // This is crucial even without drag logic for a clean tap event.
        e.preventDefault();

        const cell = e.currentTarget;
        const tappedSectionId = cell.dataset.sectionId;

        // 1. If a section is already SELECTED
        if (selectedSectionId) {
            // A. Tapped on the SAME selected section (Deselect)
            if (tappedSectionId == selectedSectionId) {
                resetMobileState();
                return;
            }

            // B. Tapped on an EMPTY cell (Move action)
            if (!tappedSectionId) {
                const sectionToMove = sections.find(
                    (s) => s.id == selectedSectionId
                );
                if (sectionToMove) {
                    updateSectionPosition(selectedSectionId, x, y);
                }
                resetMobileState();
                return;
            }

            // C. Tapped on a DIFFERENT occupied section (New Selection)
            if (tappedSectionId) {
                resetMobileState(); // Deselect the old one
                selectedSectionId = tappedSectionId;
                cell.classList.add("mobile-selected");
                return;
            }
        } else {
            // 2. No section is currently SELECTED

            // A. Tapped on an occupied section (Selection action)
            if (tappedSectionId) {
                selectedSectionId = tappedSectionId;
                cell.classList.add("mobile-selected");
                return;
            ...
            // B. Tapped on an empty cell (Do nothing)
        }
    }

    // Simplifies state cleanup after a move or deselection
    function resetMobileState() {
        document
            .querySelectorAll(".grid-cell.mobile-selected")
            .forEach((cell) => {
