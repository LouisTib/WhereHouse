document.addEventListener("DOMContentLoaded", function () {
    ...
    let initialX = null;
    let initialY = null;
    // Threshold to differentiate a tap from a drag gesture (e.g., 10 pixels)
    ...
    function generateGrid() {
        ...
        for (let y = 0; y < 20; y++) {
            for (let x = 0; x < 30; x++) {
                ...
                // Add Touch Event Listeners to grid cells for drop targets
                cell.addEventListener("touchmove", handleTouchMoveOnCell, {
                    passive: false,
                });
                cell.addEventListener("touchend", (e) =>
                    handleTouchEndOnCell(e, x, y)
                );
    ...
    function handleTouchStart(e, section) {
    ...
    function handleTouchMove(e) {
        ...
        // Check if the movement exceeds the threshold to confirm a drag
        if (
        ...
        if (isDragging) {
            // ** CRITICAL FIX: Prevent default action (scrolling/grid movement) **
            e.preventDefault();

            // Apply visual movement
            currentDraggingElement.style.transform = `translate(${deltaX}px, ${deltaY}px)`;
    ...
    // Helper for touchmove over grid cells to maintain drop target highlights
    function handleTouchMoveOnCell(e) {
        // ** CRITICAL FIX: Stop the grid's scrolling behavior while a section is active **
        if (isDragging) {
            e.preventDefault();
        }
    }

    function highlightDropTarget(targetElement) {
        document
            .querySelectorAll(".grid-cell.available-drop")
            .forEach((cell) => {
                cell.classList.remove("available-drop");
            });
    ...
    function handleTouchEnd(e) {
        if (!draggedSection || !isDragging) {
            // Not a drag, reset state and exit
            resetMobileDragState();
            return;
        }

        // Get the element at the release point
        const touch = e.changedTouches[0];
        const target = document.elementFromPoint(touch.clientX, touch.clientY);

        const dropCell = target ? target.closest(".grid-cell") : null;

        if (dropCell) {
            const x = dropCell.dataset.x;
            const y = dropCell.dataset.y;

            // Check for occupancy
            if (
                dropCell.classList.contains("occupied") &&
                dropCell.dataset.sectionId != draggedSection.id
            ) {
                alert("This cell is already occupied!");
            } else {
                updateSectionPosition(draggedSection.id, x, y);
    ...
    function resetMobileDragState() {
        if (currentDraggingElement) {
            // Clean up mobile-specific visual styles
            currentDraggingElement.classList.remove("dragging");
            currentDraggingElement.style.transform = "";
        }
    ...
    // Required for compatibility, keeps the cell from interfering
    function handleTouchEndOnCell(e, x, y) {
        // Drop logic is handled by handleTouchEnd on the dragged element.
    }
